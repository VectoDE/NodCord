  ...
  async execute(interaction) {
    ...
    collector.on('collect', async (m) => {
      ...
      if (m.stickers.size == 0)
        return await interaction.editReply('This is not a sticker.');
      ...
      if (
        ...
        return await interaction.reply(
          'I do not have permissions to add stickers to the server.'
        );
      try {
        ...
        await interaction.editReply(
          `The sticker with the name **${newSticker.name}** has been added.`
        );
      } catch (err) {
        console.log(err);
    ...
    collector.on('end', async (reason) => {
      if (reason === 'time')
        return await interaction.editReply('Ran out of time.');
    });
  },
};
