...
module.exports = {
  ...
  async execute (interaction) {
    ...
    if(!member.roles.cache.has(role.id)) {
      return await interaction.reply({ content: "Member don't have that role.", ephemeral: true });
    } else {
      member.roles.remove(role).catch(console.error);
    }

    await interaction.reply({ embeds: [new EmbedBuilder().setDescription(`**${role}** has been **removed** to **${member}**.`).setColor('Random')] });
  }
}
