...
module.exports = {
  ...
  async execute (interaction, client) {
    ...
    if(!interaction.member.permissions.has(PermissionsBitField.Flags.BanMembers)) return await interaction.reply({ content: "You don't have permissions to ban members in this server." });

    if(interaction.member.id === id) return await interaction.reply({ content: "You can't ban yourself." });
